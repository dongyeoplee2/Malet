    ...
    for k in set(base) & set(other):
        if isinstance(base[k], list):
            if base[k] and isinstance(base[k][-1], dict):
                base[k] = base[k][:-1] + other[k][:-1] + [merge_dict(base[k][-1], other[k][-1])]
...
default_style = {
    ...
    "ax_style": {
        ...
        "legend": [{"fontsize": 20}],
        "grid": [True, {"linestyle": "--"}],
        "tick_params": [{"axis": "both", "which": "major", "labelsize": 25, "direction": "in", "length": 5}],
    },
    "line_style": {
        "linewidth": 4,
        "marker": "D",
...
def ax_styler(ax: Axes, **style_dict):
    ...
    Handles special keys 'fig_size' and 'frame_width' directly, then
    delegates remaining entries to the corresponding axes methods
    (``set_<name>`` or direct attribute for tick_params, legend, grid).

    Args:
        ax: Matplotlib axes to style.
        **style_dict: Style parameters where each value is a list of
            positional args followed by a keyword-arg dict as the last element.
    """
