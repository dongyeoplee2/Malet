...
class ConfigIter:
    ...
    def __init__(self, exp_config_path: str):
        ...
        assert not (f := set(self.static_configs) & set(self.grid_fields)), (
    ...
    def grid_dict(self) -> dict:
        ...
        acc = lambda a, d: {k: [*{*castl2t(va), *castl2t(vd)}] for (k, va), (_, vd) in zip(a.items(), d.items())}
    ...
    def __sub_cmd(cfg_str):
        """Compiles list comprehension syntax in YAML config strings.

        Expands bracket expressions like ``[expr;start:step:stop]`` into
        evaluated Python lists, supporting multiple range segments joined
        by ``--``.

        Args:
            cfg_str (str): Raw YAML config string potentially containing
                list comprehension expressions.

        Returns:
            str: Config string with all list comprehension expressions
                replaced by their evaluated list representations.
        """
        # \[__;0:2:10--11:5:20]
        ...
        for entry in re.finditer(p := "\\[[^;\n]+;(\\d+:\\d+:\\d+(--)?)+\\]", cfg_str):
            ...
            assert re.sub(r"[^\+\-\*/\[\]i\d\(\), ]", "", f) == f, (
                f"Cannot use alphabet other than 'i' in expression '{entry}'"
            )
    ...
    def __extract_grid_order(cfg_str) -> List[str]:
        """Parses the ordered list of grid field names from a raw config string.

        Extracts field names appearing under the ``grid:`` section of the
        YAML string, preserving their first-occurrence order and excluding
        the ``group`` keyword.

        Args:
            cfg_str (str): Raw YAML config string.

        Returns:
            List[str]: Deduplicated, ordered list of grid field names.
        """
        if "grid" not in cfg_str:
            return []

        grid = re.split("grid ?:", cfg_str)[1]
        names = re.findall(r"[\w_]+(?= ?:)", grid)

        dupless_names = []
        for n in names:
            if n in dupless_names or n == "group":
                continue
    ...
    def __ravel_group(grid):
        """Flattens grouped fields in a grid into individual config dicts.

        Expands ``group`` entries by zipping co-indexed values (for a single
        group dict) or computing the Cartesian product (for a list of group
        dicts), then merges each combination with the remaining grid fields.

        Args:
            grid (dict): A single grid specification dict, potentially
                containing a ``group`` key.

        Returns:
            Iterable[dict]: An iterable of grid dicts with groups unraveled.
        """
        # Return list of grid if there is no 'group'
        if "group" not in grid:
            return [grid]
        group = grid["group"]
        ...
        def grid_g(g):
            ...
            assert all([l == g_len[0] for l in g_len]), (
                f"Grouped fields should have same length, got fields with length {dict(zip(g.keys(), g_len))}"
            )
    ...
    def __get_iter(self):
        """Builds the full grid iterator from the parsed config.

        Unravels grouped fields, then computes the Cartesian product of
        all grid field values to produce a list of config dicts.

        Returns:
            list[dict]: List of dicts, each mapping grid field names to
    ...
    def __getitem__(self, idx):
        """Returns a ConfigDict for an integer index, or a sliced copy of this ConfigIter.

        Args:
            idx (int | slice): Index or slice into the grid iterator.

        Returns:
            ConfigDict | ConfigIter: A single merged config dict, or a new
                ConfigIter with a sliced grid iterator.
        """
    ...
    def __len__(self):
        """Returns the number of configurations in the grid iterator."""
...
class ExperimentLog:
    ...
    def grid_dict(self) -> Dict[str, Any]:
        ...
        ```python
        {"optimizer": ["sgd", "adam"], "lr": [0.1, 0.01], "weight_decay": [0.01, 0.001]}
        ```
        ...
        return {k: __safe_sorted(set(self.df.index.get_level_values(k))) for k in self.grid_fields}
    ...
    @classmethod
    def from_fields(
        cls, grid_fields: list, metric_fields: list, static_configs: dict, logs_file: str, use_filelock: bool = False
    ) -> "ExperimentLog":
        ...
        assert not (f := set(grid_fields) & set(metric_fields)), (
            f"Overlapping field names {f} in grid_fields and metric_fields. Remove one of them."
        )
        return cls(
    ...
    def from_config_iter(
        cls, config_iter: ConfigIter, metric_fields: list, logs_file: str, use_filelock: bool = False
    ) -> "ExperimentLog":
        ...
        return cls.from_fields(
            config_iter.grid_fields, metric_fields, config_iter.static_configs, logs_file, use_filelock=use_filelock
        )

    @classmethod
    def from_tsv(cls, logs_file: str, use_filelock: bool = False, parse_str=True) -> "ExperimentLog":
        ...
        return cls(logs["df"], logs["static_configs"], logs_file, use_filelock=use_filelock)
    ...
    def from_wandb_sweep(
        ...
        for run in runs:
            ...
            if filter_dict is not None and not all((run.config[k] in vs) for k, vs in filter_dict.items()):
                continue
        ...
        try:
            ...
            list_cols = [c for c in df.columns if df[c].dropna().apply(lambda x: isinstance(x, list)).any()]

            print(df[list_cols].head())

            list_grid_fields = [c for c in grid_fields if df[c].dropna().apply(lambda x: isinstance(x, list)).any()]

            print(df[list_grid_fields].head())

            raise

        return cls(df, static_configs, logs_file=logs_file, use_filelock=False)
    ...
    def parse_tsv(cls, log_file: str, parse_str=True) -> dict:
        ...
        if parse_str:
            df = df.applymap(str2value) if not hasattr(df, "map") else df.map(str2value)

        # set grid_fields to multiindex
        df = df.set_index(idx[1:])

        return {"df": df, "grid_fields": idx[1:], "metric_fields": col, "static_configs": static_configs}

    def lock_file(func):
        """Decorator that wraps a method with filelock acquire/release.

        If ``self.use_filelock`` is True, acquires the filelock before
        calling ``func`` and releases it afterward. Otherwise calls
        ``func`` directly.
        ...
        Returns:
            Callable: Wrapped method with conditional file locking.
        """
    ...
    def __add_column(df, new_column_name: str, fn: Callable, *fn_arg_fields: str) -> pd.DataFrame:
        """Adds a new column to a DataFrame computed from existing columns.

        Applies ``fn`` row-wise to the values in ``fn_arg_fields``. If any
        argument is not a primitive type, the result is set to None.

        Args:
            df (pd.DataFrame): DataFrame to add the column to.
            new_column_name (str): Name of the new column.
            fn (Callable): Function to compute the new column value.
            *fn_arg_fields (str): Column names whose values are passed
                as arguments to ``fn``.
    ...
    def resolve_merge_conflicts(self, other: "ExperimentLog") -> Tuple["ExperimentLog", "ExperimentLog"]:
        ...
        for i, k in enumerate(same_fields):
            summary_tab.add_row(
                f"{k:{ln_k}s}",
                f"{self_d['dict'].get(k, '')!s:{ln_s}s}",
                f"{other_d['dict'].get(k, '')!s:{ln_o}s}",
        ...
        for i, k in enumerate(new_to_self):
            ...
            summary_tab.add_row(
                f"{k:{ln_k}s}",
                rd(f"{self_d['dict'].get(k, '')!s:{ln_s}s}", i),
                f"{other_d['dict'].get(k, '')!s:{ln_o}s}",
                style="on bright_black" if i % 2 else "",
        ...
        for i, k in enumerate(new_to_othr):
            ...
            summary_tab.add_row(
                f"{k:{ln_k}s}",
                f"{self_d['dict'].get(k, '')!s:{ln_s}s}",
                rd(f"{other_d['dict'].get(k, '')!s:{ln_o}s}", i),
                style="on bright_black" if i % 2 else "",
            )
        ...
        print(
            Align(
                Panel(
                    f"Detected [bold red]{len(new_to_self + new_to_othr)}[/bold red] conflicts to resolve.",
                    padding=(1, 3),
                ),
                align="center",
            )
        ...
        for i in (False, True):
            ...
            if ntt:
                print(f"\n[bold][Handle missing fields in {ts}][/bold]", f"(Default: same/first value of {fs})")
                j = 0
                while j < len(ntt):
                    k = ntt[j]
                    tab = Table()
                    ...
                    if tlog == self:
                        tab.add_row(
                            f"{k:{ln_k}s}",
                            rd(f"{self_d['dict'].get(k, '')!s:{ln_s}s}", 0),
                            f"{other_d['dict'].get(k, '')!s:{ln_o}s}",
                        )
                    elif tlog == other:
                        tab.add_row(
                            f"{k:{ln_k}s}",
                            f"{self_d['dict'].get(k, '')!s:{ln_s}s}",
                            rd(f"{other_d['dict'].get(k, '')!s:{ln_o}s}", 0),
                        )
                    ...
                    modes = ["Add new value"]
                    if ntf:
                        modes.append("merge with existing field")
                    if k in fd["sttc_d"]:
                        modes.append("remove")
                    if 0 < j < len(ntt):
                        modes.append("revert")
                    ...
                    if isinstance(mode, int):
                        if modes[mode] == "Add new value":
                            ...
                            while True:
                                ...
                                if new_field in ntf + [""]:
                                    break
                                print(f"│   There is no field:{new_field} to merge with.", f"Choose from {ntf}")
                            if new_field:
    ...
    def __cfg_match_row(self, config):
        """Finds rows in the log DataFrame matching the given config's grid fields.

        Args:
            config (Mapping[str, Any]): Configuration to match against
                the DataFrame's multi-index.

        Returns:
            pd.DataFrame: Subset of ``self.df`` whose index values match
                the config's grid field values.
        """
        if not self.grid_fields:
            return self.df
    ...
    def drop_duplicates(self):
        ...
        if len(conflict_dups) > 1:
            ...
            while True:
                ...
                if conf_handle.lower() in ["y", "yes", "n", "no"]:
                    break
            ...
            if manually_resolve:
                ...
                print("Automatically resolving conflict by keeping first row of each config.")
            ...
            for c_i, idx in enumerate(conflict_config_idxs):
                ...
                if manually_resolve:
                    ...
                    conflict_cn = [c for c in duplicate_group.columns if duplicate_group[c].nunique() > 1]
                    print(f"│  Conflicts in {len(conflict_cn)} column(s):", ", ".join(conflict_cn))
                    for i, cn in enumerate(conflict_cn):
                        ...
                        while True:
                            ...
                            if choice in [*map(str, range(len(col)))]:
                                break
                            print(f"│  Invalid input. Please enter a number between 0 and {len(col) - 1}.")
        ...
        non_duplicates = non_duplicates.map(lambda x: list(x) if isinstance(x, tuple) else x)
        print(
            Align(
                Panel(
    ...
    def melt_and_explode_metric(
        self, df: Optional[pd.DataFrame] = None, step: Optional[int] = None, dropna: bool = True
    ) -> pd.DataFrame:
        ...
        df["metric_value"] = df.apply(
            lambda df: df["metric_value"][df.step - 1] if isinstance(df["metric_value"], list) else df["metric_value"],
            axis=1,
        ...
        if dropna:
            df = df[pd.to_numeric(df["metric_value"], errors="coerce").notnull()].dropna().astype("float")
        return df
    ...
    def __str__(self):
        lines = ["[Static Configs]", *(f"{k}: {v}" for k, v in self.static_configs.items()), self.__sep, str(self.df)]
        return "\n".join(lines)
...
class RunInfo:
    ...
    def get(self):
        ...
        Returns:
            dict: Dictionary with keys ``datetime``, ``duration``, and
                ``commit_hash``.
        """
        return {"datetime": self.__datetime, "duration": self.__duration, "commit_hash": self.__commit_hash}

    def update_and_get(self):
        """Updates the accumulated duration to now and returns run info.

        Adds the elapsed time since the last update (or init) to the
        stored duration, then returns the updated run info dict.
...
class Experiment:
    ...
    def __get_and_split_configs(cfg_file, exp_bs, exp_bi, name):
        """Loads configs from a YAML file and splits them for parallel runs.

        Supports two splitting modes: uniform index-based splitting when
        ``exp_bs`` is an integer, or field-value-based splitting when
        ``exp_bs`` is a grid field name.

        Args:
            cfg_file (str): Path to the YAML experiment config file.
            exp_bs (int | str): Total number of splits (int) or grid field
                name to split on (str).
            exp_bi (int | str): Current split index (int) or space-separated
                field values to select (str).
            name (str): Experiment name for logging.

        Returns:
            ConfigIter: Filtered configuration iterator for this split.
        """
        ...
        if isinstance(exp_bs, int):
            ...
            assert exp_bs > exp_bi, (
                "Experiment split index should be smaller than the total number of experiment splits"
            )
            if exp_bs > 1:
    ...
    def __get_log(self, logs_file, metric_fields=None, filelock=False):
        """Loads an existing experiment log from file, or creates a new one.

        If the log file exists, it is loaded and resumed. Otherwise, a new
        ``ExperimentLog`` is created from the current config iterator and
        saved to disk.

        Args:
            logs_file (str): Path to the TSV log file.
            metric_fields (list | None): Metric field names for a new log.
            filelock (bool): Whether to use file locking.

        Returns:
            ExperimentLog: The loaded or newly created experiment log.
        """
    ...
    def __check_matching_static_configs(self):
        """Validates static config consistency between the config iterator and log.

        Asserts that both the keys and values of ``self.configs.static_configs``
        match those in ``self.log.static_configs``.

        Raises:
            AssertionError: If keys or values differ between the config
                iterator and the experiment log.
        """
        ...
        assert not non_match, (
            f"Found non-matching values {non_match} in static config of configiter and experiement log."
        )
    ...
    def update_log(self, config, status=None, **metric_dict):
        ...
        self.log.load_tsv()
        self.log.add_result(config, **metric_dict, **self.__curr_runinfo.update_and_get(), status=status)
        self.log.to_tsv()
    ...
    def run(self):
        ...
        for i, config in enumerate(self.configs):
            ...
            if info_dict.get("status") in {self.__RUNNING, self.__COMPLETED}:
                continue
            ...
            try:
                ...
                if self.timeout:
                    exp_func = settimeout_func(
                        exp_func, timeout=self.timeout - (datetime.now() - start_t).total_seconds()
                    )
                if self.checkpoint:
    ...
    def resplit_logs(exp_folder_path: str, target_split: int = 1, save_backup: bool = True):
        ...
        if os.path.exists(logs_folder):  # if log is splitted
            ...
            base, *logs = [
                ExperimentLog.from_tsv(os.path.join(logs_folder, sp_n), parse_str=False) for sp_n in glob.glob("*.tsv")
            ]
            base.merge(*logs)
